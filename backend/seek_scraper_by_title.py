"""
Seek Job Scraper by Job Title with ANZSCO 482 Validation
Searches for specific job titles, scrapes in parallel (3 workers), and validates eligibility
"""
...
class SeekJobTitleScraper:
    """Scraper that searches Seek.com.au by specific job titles"""
    ...
    def __init__(self, max_workers=3):
        self.jobs = []
        self.max_workers = max_workers  # Reduced to 3 to avoid rate limiting
    ...
    def fetch_job_with_description(self, job_url):
        ...
        try:
            ...
            response = StealthyFetcher.fetch(
                ...
                network_idle=False,
                load_dom=True,
                wait_selector='[data-automation="job-details"], .y735df0',  # Wait for description element
                wait=1500,
            ...
            if response.status == 200:
                # Extract description from Seek's structure
                description = None
                
                # Try multiple selectors for Seek's description container
                selectors = [
                    '[data-automation="jobAdDetails"]',
                    '[data-automation="job-details"]',
                    '.y735df0',  # Seek's common class for job details
                ...
                # Another fallback - look for any element with lots of text
                if not description or len(description) < 50:
                    all_divs = response.css('div')
                    for div in all_divs:
                        text = div.get_all_text(separator='\n', strip=True)
                        if text and len(text) > 100 and 'apply' not in text.lower()[:50]:
                            # Check if it looks like a job description (not navigation/footer)
                            lines = text.split('\n')
                            if len(lines) > 3 and any(word in text.lower() for word in ['responsibilit', 'requirement', 'experience', 'skills']):
    ...
    def scrape_job_title(self, job_title, location, max_pages=2):
        """Scrape jobs for a specific job title and location from Seek"""
        print(f"\n{'='*60}")
        print(f"Seek.com.au - Searching: {job_title} in {location}")
        print(f"{'='*60}")
        ...
        for page_num in range(1, max_pages + 1):
            ...
            try:
                ...
                response = StealthyFetcher.fetch(
                    ...
                    hide_canvas=True,
                    block_webrtc=True,
                    wait_selector='[data-automation="searchResults"]',  # Wait for results to load
                    wait=1000
                )
                ...
                # Find job cards - Seek uses specific selectors
                job_cards = response.css('[data-automation="normalJob"]')
                
                if not job_cards:
                    # Try alternative selector
                    job_cards = response.css('article[data-search-sol-meta]')
                ...
                for i, card in enumerate(job_cards, 1):
                    try:
                        # Extract title
                        title_elem = card.css_first('[data-automation="jobTitle"]')
                        title = title_elem.text.strip() if title_elem and title_elem.text else None
                        
                        if not title:
                            # Try alternative title selector
                            h3_elem = card.css_first('h3')
                            title = h3_elem.text.strip() if h3_elem and h3_elem.text else None
                        
                        # Extract company
                        company_elem = card.css_first('[data-automation="jobCompany"]')
                        company = company_elem.text.strip() if company_elem and company_elem.text else None
                        
                        if not company:
                            # Try alternative company selector
                            company_span = card.css_first('span[class*="company"]')
                            company = company_span.text.strip() if company_span and company_span.text else None
                        
                        # Extract location
                        location_elem = card.css_first('[data-automation="jobLocation"]')
                        job_location = location_elem.text.strip() if location_elem and location_elem.text else None
                        ...
                        link_selectors = [
                            ...
                            'h3 a',
                            'a[href*="/job/"]',
                            'a[data-search-sol-meta]'
                        ]
                        
                        for selector in link_selectors:
                            link_elem = card.css_first(selector)
                ...
                print(f"  Extracted {extracted} job URLs from page {page_num}")
        ...
        # Step 2: Process jobs in parallel (3 at a time)
        if not all_job_urls:
...
"""
    ...
    def scrape_multiple_titles(self, job_titles, location, max_pages=2):
        """Scrape multiple job titles from the same location"""
        all_jobs = []
        
        for i, job_title in enumerate(job_titles, 1):
            print(f"\n[Job Title {i}/{len(job_titles)}]")
            jobs = self.scrape_job_title(job_title, location, max_pages=max_pages)
...
def main():
    """Main execution function for Seek.com.au"""
    ...
    LOCATION = "Sydney-NSW"  # Seek uses hyphenated locations
    
    OUTPUT_FILE_SCRAPED = "seek_jobs_by_title_scraped.json"
    OUTPUT_FILE_VALIDATED = "seek_jobs_by_title_validated.json"
    
    print(f"{'='*60}")
    print(f"Seek.com.au Job Scraper by Title with ANZSCO 482 Validation")
    print(f"{'='*60}")
    print(f"Job Titles: {', '.join(JOB_TITLES)}")
    print(f"Location: {LOCATION}")
    print(f"Date Range: All listings")
    print(f"Parallel Workers: 3")
    print(f"{'='*60}")
    
    # Initialize scraper with 3 parallel workers
    scraper = SeekJobTitleScraper(max_workers=3)
